...
const addLocation = async (req, res) => {
  try {
    ...
    const locationCreated = await locationModel.create(req.body);
  ...
  try {
    ...
    if (accountFound != null) {
      ...
      if (Account.memberType == "HR") {
        ...
        if (hrMem[0] != null) {
          const id =
            Number(hrMem[hrMem.length - 1].memberId.substring(2, 3)) + 1;
      ...
      if (Account.memberType == "Academic") {
        ...
        if (acMem[0] != null) {
          const id =
            Number(acMem[acMem.length - 1].memberId.substring(2, 3)) + 1;
      ...
      const loc = await locationModel.findOne({
        locationName: Account.officeLocation,
      });
      if (!loc) {
        return res.json({
          statusCode: unknown,
  ...
  try {
    ...
    const newAccount = await staffModel.findOneAndUpdate(
      { memberId: Account.memberId },
      {
        ...
        staffMemberType: Account.staffMemberType,
    ...
    if (accountFound.staffMemberType == "HR Member") {
      ...
      if (accountFound.staffMemberType == "Course Coordinator") {
        await courseModel.findOneAndUpdate(
          { courseCoordinator: Account.memberId },
          { courseCoordinator: "Unassigned" }
        );
        ...
        await departmentModel.findOneAndUpdate(
          { departmentHead: Account.memberId },
          { departmentHead: "Unassigned" }
        );
        while (i < allDepts.length) {
          var dept = allDepts[i];
          if (dept.staff.includes(Account.memberId)) {
  ...
  try {
    ...
    if (deptFound.courses.includes(Course.courseId)) {
      ...
      if (index != -1) {
        ...
        await departmentModel.findOneAndUpdate(
          { departmentName: deptFound.departmentName },
          { courses: deptFound.courses }
        );
      }
    } else {
      return res.json({
...
const updateCourse = async (req, res) => {
  try {
    ...
    if (Course.newDepartmentName != null) {
      await courseModel.findOneAndUpdate(
        { courseID: Course.courseId },
        { departmentName: Course.newDepartmentName }
      );
      const dept2 = await departmentModel.findOne({
        departmentName: courseFound.departmentName,
      });
      ...
      if (index2 != -1) {
        ...
        const dept3 = await departmentModel.findOne({
          departmentName: Course.newDepartmentName,
        });
        if (Course.courseNewId != null) {
          dept3.courses.push(Course.courseNewId);
          await departmentModel.findOneAndUpdate(
            { departmentName: dept3.departmentName },
            { courses: dept3.courses }
          );
          await departmentModel.findOneAndUpdate(
            { departmentName: courseFound.departmentName },
            { courses: dept2.courses }
          );
        } else {
