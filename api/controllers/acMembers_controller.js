...
const { authenticateM2 } = require("./account_controller");
    ...
    while (i < staff.length) {
      ...
      if (accountFound.daysOff == null) {
        arr[i] =
          ...
          " This is a very hardworking person who has no daysoff";
      } else {
        arr[i] =
          " " +
          accountFound.firstName +
          ...
          accountFound.lastName +
          " " +
          accountFound.daysOff;
        console.log(arr[i]);
      }

      result +=
        accountFound.firstName.toString() +
        " " +
        accountFound.lastName.toString() +
        " " +
        accountFound.memberId.toString() +
        " " +
        accountFound.daysOff +
        " - ";
...
const viewRequests = async (req, res) => {
  try {
    ...
    while (i < staffArr.length) {
      const memberRequests = await requestModel.find({
        senderID: staffArr[i].memberId,
      });
...
const viewSchedule = async (req, res) => {
  try {
    ...
    const replacementRequests = await requestModel.find({
...
const viewReplacementRequests = async (req, res) => {
  try {
    ...
    var ReplacementRequests = await requestModel.find({
...
const sendReplacementRequests = async (req, res) => {
  try {
    ...
    await staffModel.findOneAndUpdate(
      { memberID: Body.recieverID },
      { requestLog: reciever.requestLog },
...
const hodRejectRequest = async (req, res) => {
  try {
    ...
    await requestModel.findOneAndUpdate(
      ...
      { requestStatus: reqStat.requestStatus.REJECTED }
    );
...
const coordinatorViewSlotLinking = async (req, res) => {
  try {
    ...
    var course = await courseModel.findOne({
      courseCoordinator: coordinator.email,
    });
    ...
    for (i = 0; i < tas.length; i++) {
      ...
      if (ta.requestLog.length != 0) {
        ...
        while (j < ta.requestLog.length) {
          var request = await requestModel.findOne({
            requestID: ta.requestLog[j],
          });
          console.log(request);
          if (request) {
            console.log(request);
...
const sendSlotLinkingRequest = async (req, res) => {
  try {
    ...
    const coordinator = await staffModel.findOne({
      memberId: course.courseCoordinator,
    });
    if (!coordinator) {
      return res.json({
        statusCode: accountNotFound.statusCode,
    ...
    await requestModel.create({
      senderID: myID,
      recieverID: coordinator.memberId,
      requestID: reqId,
    ...
    await staffModel.findOneAndUpdate(
      ...
      { requestLog: coordinator.requestLog },
      { returnOriginal: false }
    );
    await coordinator.save();
    console.log("ccc");

    const coordinator2 = await staffModel.findOne({
      memberId: coordinator.memberId,
    });
        ...
        while (j < TAs.length) {
          ...
          if (TAs[j] == body.memberId) {
            ...
            await courseModel.findOneAndUpdate(
              ...
              { courseCoordinator: body.memberId, courseAssistant: TAs }
            );
            await staffModel.findOneAndUpdate(
              { memberId: body.memberId },
              { staffMemberType: "Course Coordinator" }
            );
            assigned = true;
          }
          j++;
    ...
    while (i < myCourses.length) {
      if (myCourses[i].courseID == body.courseId) {
        if (myCourses[i].slots.includes(body.slotID)) {
          ...
          while (j < TAs.length) {
            if (TAs[j] == body.memberId) {
              var emptySlot = await slotModel.findOne({
                acID: "Unassigned",
                courseID: body.courseId,
              });
...
const changeDayOff = async (req, res) => {
  try {
    ...
    while (i < allCourses.length) {
      ...
      //Instructor
      if (course.courseInstructors.includes(myID)) {
        var dept = await courseModel.findOne({
        ...
        var dept1 = await departmentModel.findOne({
        ...
        await requestModel.create({
          ...
          status: "Pending",
          requestType: requestType.CHANGEDAYOFF,
          brief: req.body.Body.brief,
          requestID: req.body.Body.requestID,
        });
      }
      //TA
      ...
      //Coordinator
        ...
        var coordinator = await courseModel.findOne({
          courseCoordinator: myID,
        });
        if (!coordinator) {
          return res.json({
            statusCode: accountNotFound.statusCode,
...
const cancelStillPendingRequests = async (req, res) => {
  try {
    ...
    if (!check.staffMemberType.includes("AC")) {
      return res.send("not authorized");
    }
    ...
    while (i < myRequests.length) {
      ...
      if (request == body.requestId) {
        var RequestFound = await requestModel.findOne({
          requestID: myRequests[i],
        });
        console.log(RequestFound.status);
        console.log(RequestFound.activeDate);

        if (
          (RequestFound.status == requestStatus.PENDING &&
            RequestFound.activeDate > Date.now()) ||
          RequestFound.activeDate > Date.now()
        ) {
...
const viewStatusOfAllSubmittedRequests = async (req, res) => {
  try {
    ...
    if (
      !(
        body.criteria == requestStatus.PENDING ||
        body.criteria == requestStatus.ACCEPTED ||
        body.criteria == requestStatus.REJECTED ||
        body.criteria == "All"
      )
    ) {
      return res.json({
        statusCode: unknown,
        error:
          "Please Choose Criteria For Viewing Your Submitted Requests [All , Pending , Accepted , Rejected",
      });
    }
    if (body.criteria == "All") {
      while (i < submittedRequests.length) {
        var a = await requestModel.findOne({
          requestID: submittedRequests[i].requestID,
        });
        results[j] =
          "Request Type: " +
          a.requestType +
        ...
        while (j < course.length) {
          if (course.courseAssistant.includes(body)) {
            ...
            if (index != -1) {
              ...
              await courseModel.findOneAndUpdate(
                { courseId: body.courseID },
                { courseAssistant: course.courseAssistant },
                { returnOriginal: false }
              );
...
const submitAnyTypeOfLeaves = async (req, res) => {
  try {
    ...
    while (i < allCourses.length) {
      ...
      //Case Instructor
      if (course.courseInstructors.includes(myID)) {
        var dept = await departmentModel.findOne({
          departmentName: course.departmentName,
        });
    ...
    await staffModel.findOneAndUpdate(
      { memberId: HOD },
      { requestLog: receiver.requestLog }
    );
  ...
  try {
    ...
    if (requestFound.requestType == requestType.ACCIDENTAL) {
      ...
      if (sender.accidentalLeaveBalance < 6) {
        var senderAttendanceLog = await attendanceLogModel.findOne({
          memberId: senderId,
        });
        ...
        await requestModel.findOneAndUpdate(
          { requestID: requestFound.requestID },
        ...
        sender.notifications.push(
          "Request: " +
            requestFound.requestType +
            " - RequestID: - " +
            requestFound.requestID +
            " is " +
            requestFound.ACCEPTE
        );
        ...
        await staffModel.findOneAndUpdate(
          { memberId: senderId },
          {
            ...
            accidentalLeaveBalance: sender.accidentalLeaveBalance,
          }
        );
        var index = senderAttendanceLog.missingDays.indexOf(
          requestFound.activeDate
        );
        if (index != -1) {
        ...
        await attendanceLogModel.findOneAndUpdate(
          { memberId: senderId },
          { missingDays: senderAttendanceLog.missingDays }
        );
        sender.accidentalLeaveBalance--;
      }
    }
    ...
    //Leaves End
      ...
      return res.json({
        statusCode: unknown,
        error:
          "Replacements Should Occur Only Between Instructors Or Teacher Assistants",
      });
    }
    //Change Day Off
      ...
      if (
        ...
        return res.json({
          statusCode: unknown,
          error:
            "For Change Day Off Requests, Member Should Pass His Desired Day Off in The Brief Field",
        });
      }
      ...
      await staffModel.findOneAndUpdate(
        ...
        {
          notifications: sender.notifications,
          daysOff: sender.daysOff,
          requestLog: sender.requestLog,
        }
      );
